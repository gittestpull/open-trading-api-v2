...
├── logs/                        # [New] 날짜별 거래 로그 보관
└── tools/                       # [New] 분석 및 보조 도구
    └── analyze_trades.py        # [Analysis] 종합 매매 분석기 (v2.0)
```
...
- **물타기(Pyramiding) 전략**: 1:2:4:8 비중 배분, 최대 4단계
  - **B1 -> B2**: 평단가 -1% 하락 **OR** RSI ≤ 25 시 추가 매수
  - **B3 -> B4**: 평단가 -1% 하락 **AND** RSI ≤ 25 시 추가 매수 (비중이 큰 단계에서 리스크 관리)
- **스마트 안전 장치 (Smart Safety)**:
  - **매수 쿨다운 (3분)**: 급락 시 1~2분 만에 모든 비중이 채워지는 것을 방지하기 위해 매수 간 최소 3분 간격 유지.
  - **데이터 워밍업**: 봇 시작 시 1분봉 데이터의 유효성(RSI 10~90 범위 등)을 3회 연속 검증한 후 거래 시작.
  - **수량 불일치 감지**: 로컬 상태와 실제 계좌 수량 불일치 시 자동 동기화 및 경고.
  - **중복 매도 방지**: 매도 주문 전 실시간으로 미체결 내역을 조회하여, 이미 매도 주문이 나가 있는 경우 추가 주문을 차단.
  - **API 호출 최적화**: 잔고 및 수급 데이터 캐싱으로 호출 제한 방지.
...
- **고급 자금 및 리스크 관리 (Advanced Mode)**:
  - **동적 예산 (Dynamic Budgeting)**: 계좌 잔고가 부족할 경우 설정된 `--budget`을 실제 가용 자산(현금+보유가치)에 맞춰 자동으로 캡핑. 비중 분배(1:2:4:8)의 일관성 유지.
  - **실시간 수익금(Amount) 로그**: 수익률(%) 외에도 **미체결 손익(PNL)**과 **당일 누적 수익(Today)**을 원화/달러 금액으로 실시간 표시.
  - **순수익 계산 (Net Profit)**: 제세금(국내 0.21%, 해외 0.08%)을 100% 반영하여 실제 손에 쥐는 수익금 산출.
  - **중복 주문 방지**: 매도 트리거 발생 시 미체결 주문 조회를 선행하여 중복 매도를 방지.
  - **자가 진단 (Health Check)**: 최근 5분 내 3회 이상 에러 발생 시 시작 차단, 주문 3회 연속 실패 시 실행 자동 중단.
  - **매매 요약 기록**: 매도 완료 시 `trade_summary.txt`에 한 줄 요약 자동 저장.
...
### 7.5. `tools/analyze_trades.py` (종합 매매 분석기 v2.0)
`logs/` 폴더에 축적된 데이터를 바탕으로 오늘 하루의 성과를 입체적으로 분석합니다.
- **Financial Summary**: 시작 자산 대비 현재 자산의 증감액(PNL) 및 증감률(%) 요약.
- **Active Dashboards**: 현재 보유 중인 종목(`HOLDING`)의 실시간 평가 손익금과 단계별 물타기 현황 출력.
- **Performance Analysis**: 진입 사유(RSI, BB 등) 및 물타기 단계별 승률/수익률 통계 제공.

```bash
# 매매 분석 실행 (장중/장후 모두 가능)
...
이 봇에는 단순 지표 매매를 넘어선 고도화된 트레이딩 로직이 탑재되어 있습니다.

### 6.1. Smart Weighting (가중치 매수)
- **지지선(Support Level) 매수**: 최근 60분 내 더블 바텀 패턴 지지선 근처에서 매수 시그널 발생 시, 물량을 **1.5배** 늘려 진입합니다.
- **RSI Triple Bottom (쓰리 바닥)**: 최근 60분 내 RSI 과매도(30) 구간을 **3회 독립적으로 터치**한 후 반등하면, 가장 강력한 신호로 간주하여 물량을 **2.0배** 늘려 진입합니다. (전 단계 적용)

### 6.2. BEP 정밀 튜닝
- API에서 제공하지 않는 BEP(손익분기) 단가를 MTS와 맞추기 위해, 수수료율을 정밀 보정하여 봇의 평단가를 **비용 포함 단가(BEP)**로 표시합니다.
- 이를 통해 0.1%라도 수익 구간일 때 정확하게 익절할 수 있습니다.

### 6.3. NXT Extended Market Logic

### 6.1. NXT (After-Market) Trading 🌙
정규장(KRX) 종료 후에도 야간 시장(NXT)에서 수익 기회를 포착합니다.
- **운영 시간**: 
  - `NXT_PRE`: 08:00 ~ 08:50 (장전)
  - `NXT_POST`: 15:40 ~ 20:00 (장후)
- **전용 전략**:
  - RSI/BB와 같은 보조지표는 데이터 부족으로 **비활성화**.
  - **오직 가격(Price Action)만 보고 매매**:
    - **NXT_PRE**: 전일 종가(`prev_close`) 대비 **-1%** 하락 시 매수
    - **NXT_POST**: 당일 KRX 종가(`krx_close_today`) 대비 **-1%** 하락 시 매수
- **주문 방식**: NXT에서는 시장가(Market) 주문이 불가하므로, 자동으로 **지정가(Limit)** 주문으로 전환하여 체결시킵니다.

### 6.2. Smart Weighting (지지선 가중치 매매) 🧠
단순히 조건이 맞다고 기계적으로 사는 것이 아니라, **"확률 높은 자리"**에 비중을 더 싣습니다.
- **지지선 자동 포착**: 최근 60분 차트에서 **이중 바닥(Double Bottom)** 패턴을 형성한 강력한 지지 가격대를 실시간으로 계산합니다.
- **가중치 부스팅**: 
  - 매수 시그널 발생 시, 현재가가 지지선 근처(0.5% 이내)라면 **"찐바닥"**으로 판단.
  - 매수 비중을 **1.5배 (1.5x Multiplier)**로 자동 증액하여 수익 극대화를 노립니다.

### 6.3. Profit Safety (수익률 안전장치) 🛡️
수수료만 나가는 무의미한 매매를 원천 차단합니다.
- **BB 상단 매도 제한**: 볼린저 밴드 상단에 도달하더라도, 최소 목표 수익률(예: 2.0%)이 확보되지 않으면 **절대 매도하지 않습니다.**
- **피라미딩(물타기) 최적화**: 너무 잦은 물타기로 인한 수수료 낭비를 막기 위해 **-3.0%** 간격(권장)으로 확실한 하락 시에만 추가 매수를 진행합니다.
