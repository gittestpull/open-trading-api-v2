...
class StockScreener:
    ...
    def scan_all(self, 
                 ...
                 require_investor_flow: bool = True) -> List[dict]:
        """
        전체 조건 검색 실행
        ...
        Args:
            min_volume: 최소 거래량
            max_per: 최대 PER
            require_double_bottom: 쌍바닥 필수 여부
            require_investor_flow: 수급 조건 필수 여부
        ...
        for i, ticker in enumerate(tickers):
            try:
                ...
                # 2단계: PER 확인
                per_ok, per_value = self.check_per(ticker, max_per)
                ...
                score = 0
                if per_ok: score += 1
                if stock_data['double_bottom']: score += 1
                if stock_data['foreign_consecutive']: score += 1
                if stock_data['institution_consecutive']: score += 1
